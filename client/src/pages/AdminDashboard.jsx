...
export default function AdminDashboard({ dealer: admin, authToken, onLogout, navigate }) {
  ...
  const handleSaveExchangeAsset = async () => {
    ...
    const res = await aFetch(`/api/admin/bookings/${selectedBkg.id}/exchange-asset`, {
                      ...
                      <tbody>
                        {bkgs.filter(b => !b._placeholder && (!bkgSearch.trim() || b.name?.toLowerCase().includes(bkgSearch.toLowerCase()) || b.cnic?.includes(bkgSearch))).map(b => (
                          <tr key={b.id}
                            ...
                            <td style={{ padding: '0.875rem', fontFamily: 'monospace', fontWeight: 700, color: '#059669', fontSize: '0.8rem' }}>
                              {b.bookingRef}
                              {b.paymentPlanHistory?.length > 0 && (
                                <span title={`Payment plan modified ${b.paymentPlanHistory.length}×`} style={{ marginLeft: '0.3rem', background: '#fef3c7', border: '1px solid #fde68a', color: '#92400e', borderRadius: 9999, fontSize: '0.58rem', fontWeight: 800, padding: '0.05rem 0.35rem', verticalAlign: 'middle', fontFamily: 'sans-serif' }}>✏️</span>
                              )}
                            </td>
                            <td style={{ padding: '0.875rem' }}>
                    ...
                    <div style={{ borderTop: '1px solid #f1f5f9', paddingTop: '1rem' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.625rem' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                          ...
                          {selectedBkg.paymentPlanHistory?.length > 0 && (
                        ...
                        {!ppEditMode ? (
                          <button onClick={() => openPpEdit(selectedBkg)} style={{ background: '#fef3c7', border: '1px solid #fde68a', color: '#92400e', borderRadius: 7, padding: '0.25rem 0.6rem', fontSize: '0.7rem', fontWeight: 700, cursor: 'pointer' }}>✏️ Negotiate Terms</button>
                        ) : (
                      ...
                      {!ppEditMode ? (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem' }}>
                          {selectedBkg.paymentPlanOverride ? (
                            <>
                              {[
                                ['Negotiated Price', `PKR ${Number(selectedBkg.paymentPlanOverride.negotiatedPrice).toLocaleString('en-US')}`],
                                ['Down Payment (10%)', `PKR ${Number(selectedBkg.paymentPlanOverride.downPayment).toLocaleString('en-US')}`],
                                ['Monthly Installment', `PKR ${Number(selectedBkg.paymentPlanOverride.installmentAmount).toLocaleString('en-US')}`],
                                ['Confirmation Due', `${selectedBkg.paymentPlanOverride.confirmationDueDays} days after booking`],
                                ['Installments Start', `Month ${selectedBkg.paymentPlanOverride.installmentStartMonths}`],
                              ].map(([label, value]) => (
                              ...
                              <div style={{ background: '#fef9c3', border: '1px solid #fde68a', borderRadius: 7, padding: '0.4rem 0.625rem', fontSize: '0.78rem', color: '#78350f', marginTop: '0.25rem' }}>
                                📝 {selectedBkg.paymentPlanOverride.notes}
                              </div>
                              <div style={{ fontSize: '0.68rem', color: '#94a3b8' }}>
                                Last modified by {selectedBkg.paymentPlanOverride.changedBy} · {new Date(selectedBkg.paymentPlanOverride.changedAt).toLocaleDateString('en-PK', { month: 'short', day: 'numeric', year: 'numeric' })}
                              </div>
                            </>
                          ) : (
                            <div style={{ fontSize: '0.8rem', color: '#94a3b8' }}>Standard plan for {selectedBkg.plotSize}. Click "Negotiate Terms" to record a custom negotiated price and installment terms.</div>
                          )}
                          {selectedBkg.paymentPlanHistory?.length > 1 && (
                            <details style={{ marginTop: '0.4rem' }}>
                              <summary style={{ fontSize: '0.72rem', fontWeight: 700, color: '#64748b', cursor: 'pointer', userSelect: 'none' }}>
                                📋 View change history ({selectedBkg.paymentPlanHistory.length} entries)
                              </summary>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                          ...
                          {(() => {
                            const customAmt = ppEditForm.installmentAmount && Number(ppEditForm.installmentAmount) > 0;
                            const price = Number(ppEditForm.negotiatedPrice) || 0;
                            const instOk = !customAmt || !price || Math.abs(Number(ppEditForm.installmentAmount) * 48 - price * 0.60) <= 1; // total-based, matches server
                    ...
                    <div style={{ borderTop: '1px solid #f1f5f9', paddingTop: '1rem' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.6rem' }}>
                        <div style={{ fontSize: '0.72rem', fontWeight: 700, color: '#374151', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                          🔄 Exchange / Commodity
                          {selectedBkg.exchangeAsset && <span style={{ fontWeight: 600, fontSize: '0.68rem', color: '#059669', textTransform: 'none', background: '#d1fae5', borderRadius: 6, padding: '0.1rem 0.4rem', marginLeft: '0.4rem' }}>✓ Recorded</span>}
                        </div>
                        <button onClick={() => { setExEditMode(m => !m); setExEditForm(selectedBkg.exchangeAsset ? { ...selectedBkg.exchangeAsset } : { assetType: 'property', description: '', agreedValue: '', notes: '' }); setExEditMsg(''); }} style={{ background: 'none', border: '1px solid #e2e8f0', borderRadius: 8, padding: '0.25rem 0.6rem', fontSize: '0.72rem', cursor: 'pointer', color: '#64748b', fontWeight: 600 }}>
                          {exEditMode ? '✕ Cancel' : selectedBkg.exchangeAsset ? '✏️ Edit' : '+ Record'}
                        </button>
                      </div>
                      {!exEditMode && selectedBkg.exchangeAsset ? (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem', fontSize: '0.82rem' }}>
                          {[
                            ['Type', selectedBkg.exchangeAsset.assetType?.charAt(0).toUpperCase() + selectedBkg.exchangeAsset.assetType?.slice(1)],
                            ['Description', selectedBkg.exchangeAsset.description],
                            ['Agreed Value', 'PKR ' + Number(selectedBkg.exchangeAsset.agreedValue).toLocaleString('en-US')],
                            ['Remarks', selectedBkg.exchangeAsset.notes || '—'],
                            ['Recorded by', selectedBkg.exchangeAsset.recordedBy + ' · ' + new Date(selectedBkg.exchangeAsset.recordedAt).toLocaleDateString('en-PK', { month: 'short', day: 'numeric', year: 'numeric' })],
                          ].map(([k, v]) => (
                          ...
                          <div style={{ marginTop: '0.3rem', background: '#eff6ff', borderRadius: 8, padding: '0.4rem 0.65rem', fontSize: '0.75rem', color: '#1d4ed8' }}>
                            Remaining to settle in installments: PKR {Math.max(0, (selectedBkg.originalPlotPrice || selectedBkg.plotPrice) - selectedBkg.exchangeAsset.agreedValue).toLocaleString('en-US')}
                          </div>
                        </div>
                      ) : !exEditMode ? (
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                          ...
                          <div>
                            ...
                            {exEditForm.agreedValue && Number(exEditForm.agreedValue) > 0 && (
                              <div style={{ fontSize: '0.72rem', color: '#059669', fontWeight: 600, marginTop: '0.25rem' }}>
                                → Remaining after exchange: PKR {Math.max(0, (selectedBkg.originalPlotPrice || selectedBkg.plotPrice) - Number(exEditForm.agreedValue)).toLocaleString('en-US')} to settle in installments
                              </div>
                            )}
                          </div>
                          <div>
                          ...
                          <button onClick={handleSaveExchangeAsset} disabled={exEditSaving} style={{ background: exEditSaving ? '#94a3b8' : '#0284c7', color: '#fff', border: 'none', borderRadius: 10, padding: '0.65rem 1rem', fontWeight: 700, cursor: exEditSaving ? 'not-allowed' : 'pointer', fontSize: '0.875rem' }}>
